...
echo "${bldgrn}  \(    )/     \(       \(       )/    \(      )/    \(     )/       \(    )/    ${txtrst}"
echo "${bldgrn}   '    '       '        '       '      '      '      '     '         '    '     ${txtrst}"
echo "${bldgrn}                          GCC 7.0 with Linaro patches                            ${txtrst}"
echo ""

cd ../gcc/gcc-LINARO && rm -rf * && git reset --hard && git fetch mygh gcc7 && git checkout FETCH_HEAD && cd ../../;
...
export UBER_PATH=$DIR/out/aarch64-7.0;
...
echo ""
echo "${bldblu}Building your Linaro aarch64-7.x Toolchain!!!${txtrst}"
echo ""
all1=$(date +%s.%N)
script -q $DIR/out/Linaro-AARCH64-7.x.log -c "make 1>/dev/null $MAKE_FLAGS";
...
then
    ...
    echo "${bldgrn}░                                                                  ${txtrst}"
    echo ""
    echo "${bldgrn}Your Linaro 7.x aarch64 Toolchain has completed successfully!!! ${txtrst}"
    ...
    echo "${bldred}                               ▀                                 ${txtrst}"
    echo ""
    echo "${bldred}Error Log is found at:${txtrst}${red} $DIR/out/Linaro-AARCH64-7.x.log ${txtrst}"
    echo ""
